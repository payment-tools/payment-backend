...
class SalesControllerTest extends BaseControllerTest {
    ...
    void getSalesByFilters_returns200() throws Exception {
        when(salesService.getSalesByFilters(any(), any(), any(), any(), any(), any(), any(), any(), any()))
    ...
    @Test
    void getSalesById_returns200() throws Exception {
        when(salesService.getSalesById(1L)).thenReturn(mock(SalesDTO.class));

        mockMvc.perform(get("/v1/onepay/sales/1"))
                .andExpect(status().isOk());
    }
